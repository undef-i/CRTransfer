...
#[derive(Debug, Clone)]
struct St {
    tdur: i32, aat: i32, sid: usize, idt: i32, x: i32,
...
impl Ord for St {
    fn cmp(&self, o: &Self) -> Ordering {
        o.tdur.cmp(&self.tdur)
         .then_with(|| self.idt.cmp(&o.idt))
    }
}
...
fn pre_dat(s: &str) -> Result<HashMap<usize, Vec<R>>, String> {
    ...
    for train in root.t {
        for i in 0..(train.s.len().saturating_sub(1)) {
            let b_stop = &train.s[i];
            let a_stop = &train.s[i + 1];

            if b_stop.d != -1 && a_stop.a != -1 && a_stop.a > b_stop.d {
                let b_sid = get_oc_sid(&b_stop.n);
                let a_sid = get_oc_sid(&a_stop.n);
                let duration = a_stop.a - b_stop.d;
                
                rfs.entry(b_sid).or_insert_with(Vec::new).push(R {
                    tn: train.tn.clone(),
                    bs: b_sid,
                    al: a_sid,
                    dtr: b_stop.d,
...
pub fn find(o: &str, d: &str, mtt: i32, esc_o: bool, esc_d: bool) -> Result<(), JsValue> {
    ...
    while let Some(c) = pq.pop() {
        ...
        let c_arr = c.aat; let c_dep = c.idt; let c_x = c.x;
